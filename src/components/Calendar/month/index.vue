<template>
  <div :key="year" class="wrap-calendar-month">
    <h1 class="month-title">{{ currentMonth }}月</h1>
        ...
        <span
          ...
          :class="{
            'day-high-light':
              dayItem.content &&
              startDay &&
              filterDayHighLight(dayItem.content),
            'day-high-light-error':
              dayItem.content &&
              errorDay &&
              filterDayHighLightError(dayItem.content),
          }"
        >
          {{ dayItem.content }}
        </span>
...
const useCalendarMonth = (
  ...
  const handleMonthDays = (currentMonthDays, startWeek) => {
    return handleMonthEmptyDays(currentMonthDays, startWeek).map(
      (dayItem, index) => {
        if (index >= startWeek) {
          return {
