...
/**
 * @param {*} flatteningRoutes
 * @description 路由扁平化 
 */

export const flatteningRoutes = (routes,permission)=>{
  const routes_ = routes.filter(route => permission.includes(route.meta.roles))
  routes_.forEach(route => route.children && (route.children = flatteningRoutes(route.children,permission)))
  return routes_
}