<template>
  <div class="btns">
    <el-alert
      title="该列表是当前岗位自动通过角色获得权限的用户，不可编辑！"
      show-icon
      :closable="false"
      class="alert_info"
    />
    <el-button type="primary" @click="roleSelectionFn"> 角色选择 </el-button>
  </div>
  <el-table
    v-loading="loading"
  ...
  >
    ...
    <template #empty>
      <p class="empty">
        暂无数据，请 <span class="add" @click="roleSelectionFn">选择角色</span>
      </p>
    </template>
  </el-table>
</template>
...
const roleSelectionFn = () => {
  const newData = ref([]);
  if (tableData.value.length) {
    tableData.value.forEach((item) => {
      newData.value.push(item.asorgrole);
    });
  }
  const setData = ref([]);
  ...
  proxy.$ModalDialog({
    ...
    onConfirm: async (instance) => {
      ...
      if (isValid) {
        if (data.value.roleList.length) {
          loading.value = true;
          const { error, result } = await newPermissionStore.askUserListByRole({
            code: data.value.roleList,
          });
          if (!error) {
